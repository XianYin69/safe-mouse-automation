...
双击打开，纯鼠标定位不用命令行）、**应用直接操作通道**（`app_ops.py` UIA 枚举/Invoke/SetValue +
经典 Win32 菜单经 WM_COMMAND 零鼠标执行）、**学习功能**（`learn.py` 把"软件在哪、怎么操作"沉淀到
...
5. **后台执行操作**：
   ...
   - **直接操作应用界面**：[`scripts/app_ops.py`](scripts/app_ops.py) `controls <窗口>` 枚举 UIA 控件
     （按钮/输入框/标签页等，含屏幕坐标）、`act <窗口> <元素名>` UIA Invoke/Toggle/Select 点击、
     `set <窗口> <元素名> <文本>` ValuePattern 填输入框、`menus <窗口>` 枚举经典 Win32 菜单树、
     `menu <窗口> <一级/二级>` 经 WM_COMMAND 零鼠标执行菜单命令（如 `文件/打开`）。
